    ...
    switch (this.config.projectType) {
      case CONST.SPA:
        if (this.config.framework === CONST.REACT) {
          url = 'xg-a06/react_spa_tpl';
        } else if (this.config.framework === CONST.VUE) {
          url = 'xg-a06/vue_spa_tpl';
        }
        // url = 'github:xg-a06/#master'
        break;
      case CONST.LIBRARY:
        if (this.config.buildTool === CONST.WEBPACK) {
          url = 'xg-a06/lib_tpl';
        } else {
          url = 'xg-a06/lib_vite_tpl';
        }
        break;
      default:
        break;
  ...
  downloadTpl() {
    ...
    return new Promise((resolve, reject) => {
      download(this.getDownloadUrl(), this.temp, { clone: true }, (err) => {
        spinner.stop();
        if (err) {
          console.error(logSymbols.error, chalk.red(`下载失败：${err}`));
          reject(err);
        } else {
